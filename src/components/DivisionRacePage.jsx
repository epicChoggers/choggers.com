...
import DivisionRaceSection from './DivisionRaceSection'
import { fetchDivisionRaceData, fetchMarinersHomeGamesSinceOpeningDay, fetchSeasonStats, fetchHRData } from '../utils/data'

function DivisionRacePage() {
  const [divisionData, setDivisionData] = useState({ results: {}, summary: [] })
  ...
  useEffect(() => {
    const initializePage = async () => {
      try {
        ...
        // Fetch division race data, robbery count, and header stats
        const [data, robberyData, seasonStats, hrData] = await Promise.all([
          fetchDivisionRaceData(),
        ...
        console.error('Failed to initialize division race page:', err)
        setError('Failed to load division race data. Please try refreshing the page.')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading && divisionData.summary.length === 0) {
    return (
      <div className="app">
        <Header 
...
export default DivisionRacePage 